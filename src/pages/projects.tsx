...
export default function Projects({
    ...
    let activeProjectsInSearch = 0;
    let inactiveProjectsInSearch = 0;

    return (
        <div>
            <SearchBar
                setExternalSearchTerm={setSearchTerm}
                setExternalFilterType={setFilterType}
            />
            <div className="flex flex-wrap justify-center min-h-screen">
                ...
                {!enableSearch ? (
                    <div id="defaultWithoutSearch" className="m-0">
                        ...
                        {activeCount > 0 ? (
                            <div>
                                <div className=" p-2 bg-green-500 flex">
                                    <h1 className="m-4">Active Projects</h1>
                                </div>
                        ...
                        {inactiveCount > 0 ? (
                            <div>
                                <div className=" p-2 bg-red-500 flex">
                                    <h1 className="m-4">
                        ...
                        {activeCount > 0 ? (
                                ...
                                <div className="flex flex-wrap justify-center">
                                    {filterType != ""
                                        ? Object.keys(activeProjects).map((key) => {
                                            if (
                                                (filterType == "option1" &&
                            activeProjects[key]["mentor_last_name"].includes(
                                searchTerm,
                            )) ||
                          (filterType == "option2" && key.includes(searchTerm))
                                    ...
                                    {activeProjectsInSearch == 0 ? (
                                        <p className="p-20">
                                            {" "}
                                            No active projects for your search term{" "}
                                            <b>"{searchTerm}"</b>
                                        </p>
                                    ) : null}
                                </div>
                                ...
                                <div className="flex flex-wrap justify-center">
                                    {filterType != ""
                                        ? Object.keys(inactiveProjects).map((key) => {
                                            if (
                                                (filterType == "option1" &&
                            inactiveProjects[key]["mentor_last_name"].includes(
                                searchTerm,
                            )) ||
                          (filterType == "option2" && key.includes(searchTerm))
                                            ) {
                                    ...
                                    {inactiveProjectsInSearch == 0 ? (
                                        <p className="p-20">
                                            {" "}
                                            No inactive projects for your search term{" "}
                                            <b>"{searchTerm}"</b>
                                        </p>
                                    ) : null}
                                </div>
...
export async function getStaticProps() {
    ...
    let projects = YAML.parse(fileContents);
    ...
    for (const key in projects) {
        ...
        branch,
            ...
            .catch((error) => {
                console.log(
                    "Odds are your query is being throttled. Try again in a few minutes.",
                );
            });
    }
    ...
    for (const key in projects) {
        const date = new Date(projects[key]["date"]);
